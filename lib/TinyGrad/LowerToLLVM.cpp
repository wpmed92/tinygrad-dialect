...
public:
  explicit PrintOpLowering(mlir::MLIRContext *context)
    : mlir::ConversionPattern(tinygrad::PrintOp::getOperationName(), 1, context) {}
...
namespace {
class TinyGradToLLVMLoweringPass
        : public mlir::PassWrapper<TinyGradToLLVMLoweringPass, mlir::OperationPass<mlir::ModuleOp>> {
public:
  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(TinyGradToLLVMLoweringPass)
...
void TinyGradToLLVMLoweringPass::runOnOperation() {
  ...
  patterns.add<tinygrad::PrintOpLowering>(&getContext());
...
std::unique_ptr<mlir::Pass> tinygrad::createLowerToLLVMPass() {
  return std::make_unique<TinyGradToLLVMLoweringPass>();
}